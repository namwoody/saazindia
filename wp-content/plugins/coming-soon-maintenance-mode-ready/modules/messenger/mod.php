...
class messengerCsp extends moduleCsp {
	...
	public function displayModActivationPopup() {
		$this->getController()->getView()->displayModActivationPopup();
	}
    public function mailContentType($content_type) {
        $content_type = 'text/html';
        return $content_type;
    }
	// We should call this only if send messages from plugin - as it can cause problems for mails, that should b sent using another content type
	public function addContentTypeFilter() {
		add_filter('wp_mail_content_type', array($this, 'mailContentType'));
	}
    /**
     *
