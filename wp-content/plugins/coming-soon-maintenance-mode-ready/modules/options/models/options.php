...
class optionsModelCsp extends modelCsp {
	...
	/**
	 * public alias for _setByCode()
	 */
	public function setByCode($code, $value) {
		return $this->_setByCode($code, $value);
	}
	...
	public function setTplAnyDefault($d = array()) {
		...
		switch($code) {
			...
			case 'slider_images':
				$newOptValue = false;
				if(frameCsp::_()->getModule('bg_slider')) {
					if(frameCsp::_()->getModule('bg_slider')->getController()->getModel()->saveTplDefaultSlides()) {
						$newOptValue = array(
							'slides' => frameCsp::_()->getModule('bg_slider')->getSlidesFullPath(),
							'slidesNames' => frameCsp::_()->getModule('options')->get('slider_images'),
						);
					} else
						$this->pushError( frameCsp::_()->getModule('bg_slider')->getController()->getModel()->getErrors() );
				} else {
					$this->pushError(langCsp::_('There are no Bg slider module installed'));
				}
				break;
			default:
		...
		if(!empty($code)) {
			...
			if($plTemplate && frameCsp::_()->getModule($plTemplate)) {
				...
				if($newValue !== NULL && file_exists(frameCsp::_()->getModule($plTemplate)->getModDir(). $newValue)) {
					// Remove prev. image
					if(utilsCsp::fileExists( frameCsp::_()->getModule('options')->getBgImgFullDir() ))
						utilsCsp::deleteFile( frameCsp::_()->getModule('options')->getBgImgFullDir() );
