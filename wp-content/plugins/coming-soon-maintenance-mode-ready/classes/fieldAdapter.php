...
class fieldAdapterCsp {
    ...
    static public function _($fieldOrValue, $method, $type) {
        if(method_exists('fieldAdapterCsp', $method)) {
            switch($type) {
                case self::DB:
    ...
    static public function userFieldDestHtml($field) {
        $field->htmlParams['optionsCsp'] = array();
    ...
    static public function getCountries($notSelected = false) {
        ...
        if(empty($options[ $notSelected ])) {
            ...
            if(empty(self::$countries))
                self::$countries = self::getCachedCountries();
            if($notSelected) {
				$options[ $notSelected ][0] = is_bool($notSelected) ? langCsp::_('Not selected') : langCsp::_($notSelected);
			}
    ...
    static public function getStates($notSelected = false) {
        ...
        if(empty($options[ $notSelected ])) {
            ...
            if($notSelected) {
				...
				$options[ $notSelected ][0] = array('name' => langCsp::_( $notSelectedLabel ), 'country_id' => NULL);
			}
    ...
    static public function getExtraFieldOptions($field_id) {
        ...
        $options = frameCsp::_()->getModule('optionsCsp')->getHelper()->getOptions($field_id);
        if (!empty($options)) {
    ...
    static public function getFieldAttributes($params){
        ...
        if (!empty($params->attr)) {
            foreach ($params->attr as $key=>$value) {
                $output .= langCsp::_($key).':<br />';
                $output .= htmlCsp::text('params[attr]['.$key.']',array('value'=>$value)).'<br />';
            }
        } else {
                $output .= langCsp::_('class').':<br />';
                $output .= htmlCsp::text('params[attr][class]',array('value'=>'')).'<br />';
                $output .= langCsp::_('id').':<br />';
                $output .= htmlCsp::text('params[attr][id]',array('value'=>'')).'<br />';
        }
        return $output;
    }
    /**
    ...
    static function productFieldCategories($field){
        if(!empty($field->htmlParams['optionsCsp']))
            return;
        ...
        if(!empty($categories)) {
            if(!is_array($field->value['categories']))
                    $field->value['categories'] = array();
            $field->htmlParams['optionsCsp'][0] = in_array(0,$field->value['categories'])?langCsp::_('Deselect All'):langCsp::_('Select All');
            foreach($categories as $c) {
                $field->htmlParams['optionsCsp'][$c->term_taxonomy_id] = $c->cat_name;
            }
        }*/
    }
	...
	static public function getCachedCountries($clearCache = false) {
		if(empty(self::$countries) || $clearCache)
			self::$countries = frameCsp::_()->getTable('countries')->getAll('id, name, iso_code_2, iso_code_3');
	...
	static public function getCachedStates($clearCache = false) {
		if(empty(self::$states) || $clearCache)
			self::$states = frameCsp::_()->getTable('states')
				->leftJoin( frameCsp::_()->getTable('countries'), 'country_id' )
