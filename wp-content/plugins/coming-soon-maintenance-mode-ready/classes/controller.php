...
abstract class controllerCsp {
	...
	protected function _createModel($name = '') {
		...
		$className = '';
		if(importCsp($parentModule->getModDir(). 'models'. DS. $name. '.php')) {
			$className = toeGetClassNameCsp($name. 'Model');
		}
	 ...
	 * @example :
	 return array(
			S_METHODS => array(
				'save' => array(CSP_ADMIN),
				'remove' => array(CSP_ADMIN),
				'restore' => CSP_ADMIN,
			),
			S_USERLEVELS => array(
	 ...
	 * Can be used on of sub-array - CSP_METHODS or CSP_USERLEVELS
	 */
