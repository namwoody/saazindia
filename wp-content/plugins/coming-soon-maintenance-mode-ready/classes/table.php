...
abstract class tableCsp {
    ...
    static public function getInstance($table = '') {
        ...
        if(!isset($instances[$table])) {
            $class = 'table'. strFirstUp($table). strFirstUp(CSP_CODE);
            ...
            if($id) {
                ...
                foreach($data as $field) {
                    ...
                    foreach($field as $k => $v) {
                        if(isset($this->_fields[$k])) {
                            $row[$k] = toeCreateObjCsp('fieldCsp', array(
                                    $this->_fields[$k]->name,
                                    $this->_fields[$k]->html,
                                    $this->_fields[$k]->type,
    ...
    public function getTable($transform = false) {
        if($transform)
            return dbCsp::prepareQuery($this->_table);
        else
            return $this->_table;
    }
    ...
    public function store($data, $method = 'INSERT', $where = '') {
        ...
        if(empty($fields)) {
            $this->_addError(langCsp::_('Nothig to update'));
            return false;
        }
        ...
        if(dbCsp::query($query)) {
            if($method == 'INSERT')
                return dbCsp::lastID();
            else
                return true;
        } else
			$this->_addError(langCsp::_('Database error. Please contact your developer.'));
        return false;
    }
    ...
    public function exists($value, $field = '') {
        if(!$field)
            $field = $this->_id;
        return dbCsp::get('SELECT '. $this->_id. ' FROM '. $this->_table. ' WHERE '. $field. ' = "'. $value. '"', 'one');
    }
