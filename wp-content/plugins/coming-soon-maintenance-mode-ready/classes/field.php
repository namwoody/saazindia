...
class fieldCsp {
	 ...
	 */
	public $adapt = array('htmlCsp' => '', 'dbFrom' => '', 'dbTo' => '');
	/**
	...
	public function setValue($value, $fromDB = false) {
		if(isset($this->adapt['dbFrom']) && $this->adapt['dbFrom'] && $fromDB)
			$value = fieldAdapterCsp::_($value, $this->adapt['dbFrom'], fieldAdapterCsp::DB);
		$this->value = $value;
	}
	...
	public function viewField($name, $fieldValue = '') {
		$method = $this->html;
		$options = frameCsp::_()->getModule('optionsCsp')->getHelper()->getOptions($this->id);
		$attrs = '';
		...
		$params = array('optionsCsp'=>$options, 'attrs' => $attrs, 'value' => $fieldValue, 'checked' => $checked);
		$output = '';
		if(method_exists('htmlCsp', $method)) {
			$output .= htmlCsp::$method($name, $params);
			$output .= htmlCsp::hidden('extra_field['.$this->name.']',array('value'=>$this->id));
		}
		return $output;
	}
	...
	public function processParams($tag, $id){
		...
		if ($this->name == "params") {
			...
			switch ($tag) {
				case 5: 
					$add_option = langCsp::_('Add Checkbox');
					$options_tag = '';
					$image_tag = ' style="display:none"';
				break;
				case 9: 
					$add_option = langCsp::_('Add Item');
					$options_tag = '';
					$image_tag = ' style="display:none"';
				break;
				case 12:
				...
				case 10:
					$options_tag = '';
					$add_option = langCsp::_('Add Radio Button');
					$image_tag = ' style="display:none"';
				break;
				case 8:
					$image_tag = '';
			...
			if ($tag > 0 || $id == 0) {
				$output .= '<div class="options options_tag"'.$options_tag.'>';
					...
					$output .= fieldAdapterCsp::_($id,'getExtraFieldOptions',fieldAdapterCsp::STR);
				$output .= '</div>';

				$output .= '<div class="options image_tag"'.$image_tag.'>'.langCsp::_('Dimensions').':<br />';
					$params->width?$width = $params->width:'';
					$params->height?$height = $params->height:'';
					$output .= langCsp::_('width').':<br />';
					$output .= htmlCsp::text('params[width]',array('value'=>$width)).'<br />';
					$output .= langCsp::_('height').':<br />';
					$output .= htmlCsp::text('params[height]',array('value'=>$height)).'<br />';
				$output .= '</div>';
			}
			if($this->adapt['htmlParams']) {
				$output .= fieldAdapterCsp::_($this, $this->adapt['htmlParams'], fieldAdapterCsp::STR);
			} else {
				$output .= '<a href="javascript:void(0);" class="set_properties">'.langCsp::_('Click to set field "id" and "class"').'</a>';
				$output .= '<div class="attributes" style="display:none;">'.langCsp::_('Attributes').':<br />';
				$output .= fieldAdapterCsp::_($params,'getFieldAttributes',  fieldAdapterCsp::STR);
				$output .= '</div>';
			}
			return $output;
		}
	  ...
	  // load xml structure of parameters
	   ...
	   foreach ($config->params->param as $param) {
		 // read the variables
		  ...
		  $helper = $this->checkVarFromParam($param,'helperCsp');
		  $module = $this->checkVarFromParam($param,'moduleCsp');
