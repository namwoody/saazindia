...
class dbCsp {
     ...
     * #__  Wordpress prefix
     * ^__  Store plugin tables prefix (@see CSP_DB_PREF if config.php)
    ...
    static public function prepareQuery($query) {
        ...
        return str_replace(
                array('#__', '^__', '@__'), 
                array($wpdb->prefix, CSP_DB_PREF, $wpdb->prefix. CSP_DB_PREF),
                $query);
    }
    ...
    static public function timeToDate($timestamp = 0) {
        if($timestamp) {
            if(!is_numeric($timestamp))
                $timestamp = dateToTimestampCsp($timestamp);
    ...
    static public function dateToTime($date) {
        ...
        if(strpos($date, CSP_DATE_DL)) return dateToTimestampCsp($date);
        $arr = explode('-', $date);
        return dateToTimestampCsp($arr[2]. CSP_DATE_DL. $arr[1]. CSP_DATE_DL. $arr[0]);
    }
