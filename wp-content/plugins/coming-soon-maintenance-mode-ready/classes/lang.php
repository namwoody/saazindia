...
class langCsp {
	...
	static public function extract($d = array('dir' => '', 'langCsp' => '')) {
		$data = array();
		if(isset($d['dir']) && !empty($d['dir']))
			$langDirPath = $d['dir'];
		else if(isset($d['langCsp']) && !empty($d['langCsp']))
			$langDirPath = CSP_LANG_DIR. $d['langCsp']. DS;
		else
			$langDirPath = CSP_LANG_DIR. CSP_WPLANG. DS;
